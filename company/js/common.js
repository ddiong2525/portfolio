...
function updatePagination(swiper) {
    const currentNum = swiper.realIndex + 1;//현재 슬라이드 번호(0부터 시작이라 +1함)
    const formattedNum = currentNum < 10 ? `0${currentNum}` : currentNum/* 01,02,03 이런 식으로 뜨게 조건 설정 */
    document.querySelector('.swiper-pagination-current').textContent = formattedNum;
}

//마우스 따라다니는 커서+드래그 효과 만들기
...
visionCards.forEach((card, i) => {
    ...
    tl.fromTo(card, { rotationY: 0 }, {
        rotationY: 180,
        transformOrigin: "center center",/* 도는 것의 기준점 */
        ease: "power2.out"/* 가속도 */
    })
})
...
gsap.timeline({
    scrollTrigger: {
        trigger: '.vid_box',
        start: 'top 10%',
        end: "top 10%+=1500", /* 1500pixel 스크롤동안 애니메이션이 걸림 */
        scrub: 1,
        pin: '.vid_box', /* 고정시키는 거임 */
        pinSpacing: true,
...
gsap.timeline({
    scrollTrigger: {
        ...
        scrub: 1, //스크롤을 움직이면 애니메이션도 같이 부드럽게 따라가게 만듦
    ...
    .to(cards, { y: -300, z: -200, skewY: 5, duration: 1, ease: "power2.in", stagger: 1, }, '+=0.5')
